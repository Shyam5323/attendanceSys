...
document.getElementById('registerForm')?.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (detections.length === 0) {
      throw new Error('No face detected. Please try again with clearer lighting.');
    }
    
    if (detections.length > 1) {
      throw new Error('Multiple faces detected. Please ensure only one face is in frame.');
    }
...
document.getElementById('exportBtn')?.addEventListener('click', () => {
  ...
  window.reportData.forEach(record => {
    csvContent += `"${new Date(record.date).toLocaleDateString()}",` +
                  `"${record.studentId}","${record.studentName}",` +
                  `"${record.department}","${record.timeSlot === 'morning' ? '9:00 AM' : '2:00 PM'}"\n`;
  });
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (token) {
    ...
    document.getElementById('userEmail').textContent = JSON.parse(atob(token.split('.')[1]))?.email || 'Admin';
  ...
  try {
    ...
    showError('loginError', 'Failed to initialize face recognition. Please check console for details.');
  }
  ...
  document.getElementById('loginForm')?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      errorEl.textContent = err.response?.data?.error || 'Login failed. Check credentials.';
    } finally {
      btn.disabled = false;
    }
  });
